...
const STATE_KEY = "bookmaker_state_v11";
...
function importHtmlAsChapters(html) {
  ...
  function isHeading(el) {
    if (/^H[1-3]$/i.test(el.tagName)) return true;

    // Detect bold-only paragraph as heading (common in Word)
    if (el.tagName === "P") {
  ...
  for (const el of elements) {
    if (isHeading(el)) {
      ...
      current = {
        id: uid(),
        title: el.innerText.trim() || "Untitled Chapter",
        content: [],
        wordCount: 0
      };
    } else {
      if (!current) {
        current = { id: uid(), title: "Introduction", content: [], wordCount: 0 };
      }
  ...
  if (!chapters.length) {
    alert("No chapters detected. Use Heading styles or bold chapter titles.");
    return;
  }
  ...
  alert(`Imported ${chapters.length} chapters from DOCX.`);
}
